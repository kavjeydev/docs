...
<Info>
  Following these practices will help you build robust, scalable applications with Trainly.
</Info>
...
async function robustQuery(question: string, maxRetries: number = 3) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (error instanceof TrainlyError && error.status >= 500 && attempt < maxRetries) {
        // Retry server errors
        const delay = Math.pow(2, attempt) * 1000;
...
```typescript
function selectOptimalModel(questionComplexity: 'simple' | 'medium' | 'complex'): string {
  const modelStrategy = {
...
function assessComplexity(question: string): 'simple' | 'medium' | 'complex' {
  ...
  if (length < 50 && !keywords.some(k => question.toLowerCase().includes(k))) {
    return 'simple';
  }
...
export async function healthCheck(): Promise<{
  ...
  try {
    ...
    const response = await fetch('https://api.trainly.com/v1/health');
...
<CardGroup cols={2}>
  ...
  <Card title="Cost Optimization" icon="dollar-sign" href="/credits-and-pricing">
