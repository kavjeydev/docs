...
def test_query_specific_file_content(client: TrainlyClient):
    """Test 13: Query Specific File Content and Verify Answer"""
    print_test("Query Specific File Content and Verify Answer")

    try:
        # First, delete all existing files to start with a clean slate
        print_info("Cleaning up: Deleting all existing files...")
        list_result = client.list_files()
        if list_result.files:
            for file_info in list_result.files:
                try:
                    client.delete_file(file_info.file_id)
                    print_info(f"  Deleted: {file_info.filename}")
                except Exception as e:
                    print_info(f"  Could not delete {file_info.filename}: {e}")
            print_success(f"✓ Cleaned up {len(list_result.files)} existing files")
        else:
            print_info("No existing files to clean up")
        ...
        try:
            ...
            upload_result = client.upload_file(temp_path)
            file_id = upload_result.file_id
            print_success(f"Uploaded file with specific content: {upload_result.filename}")

            # Wait a moment for processing (if needed)
            import time
            time.sleep(30)
            print_info("File processed and indexed, continuing with tests...")

            # Test 1: Query about the product name
            print_info("Testing query about product name...")
            ...
            print_success("✓ Product name query verified")

            # Test 2: Query about the price
            print_info("Testing query about price...")
            ...
            print_success("✓ Price query verified")

            # Test 3: Query about specifications
            print_info("Testing query about battery life...")
            ...
            print_success("✓ Battery life specification query verified")

            # Test 4: Query about release date
            print_info("Testing query about release date...")
            ...
            print_success("✓ Release date query verified")

            # Test 5: Query about warranty
            print_info("Testing query about warranty...")
            ...
            print_success("✓ Warranty query verified")
            print_info(f"Response: {response5.answer}")

            print_success("All content verification checks passed!")

            # Test 6: Delete the file and verify AI no longer knows the content
            print_info("Testing query after file deletion...")
            delete_result = client.delete_file(file_id)
            assert delete_result.success is True, "File deletion should succeed"
            print_success(f"✓ File deleted: {delete_result.filename}")
            ...
            if has_no_knowledge or does_not_contain_price:
                print_success("✓ AI correctly indicates it doesn't know after file deletion")
                print_info(f"Response after deletion: {response_after_delete.answer}")
            else:
                print_error(f"AI still claims to know the information after deletion. Response: {response_after_delete.answer}")
                # Don't fail the test completely, but warn
                print_info("This might indicate the file hasn't been fully removed from the index yet")
            ...
            if file_id:
                try:
                    ...
                    print_info(f"Cleaned up test file: {file_id}")
                except Exception as e:
                    print_info(f"Could not delete test file: {e}")

    except AssertionError as e:
        print_error(f"Content verification failed: {e}")
        return False
    except Exception as e:
        print_error(f"Query specific content test failed: {e}")
        return False
...
def main():
    ...
    # Wait for backend processing before final test
    import time
    print_info("Waiting 10 seconds for backend processing...")
    time.sleep(10)

    # Test 13: Query specific file content
    results.append(("Query Specific File Content", test_query_specific_file_content(client)))
