...
```bash
curl -X POST "https://api.trainly.com/cleanup_chat_data/chat_abc123?child_chat_ids=subchat1,subchat2" \
  -H "Authorization: Bearer your_api_key"
```
...
async function deleteFile(fileId, userToken, appId) {
  ...
  try {
    const response = await fetch(
      `https://api.trainly.com/v1/me/chats/files/${fileId}`,
      {
        method: 'DELETE',
        headers: {
...
function FileList({ files, userToken, appId, onFileDeleted }) {
  ...
  const handleDelete = async (fileId, filename) => {
    ...
    try {
      ...
      alert(`File deleted successfully. Freed ${formatBytes(result.size_bytes_freed)}`);
