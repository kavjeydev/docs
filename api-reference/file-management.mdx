...
curl -X POST https://api.trainly.com/create_nodes_and_embeddings \
...
curl -X DELETE https://api.trainly.com/remove_context/doc_abc123 \
...
curl -X DELETE https://api.trainly.com/delete_chat_nodes/chat_abc123 \
  -H "Authorization: Bearer your_api_key"
```
...
curl -X POST "https://api.trainly.com/cleanup_chat_data/chat_abc123?convex_id=chat_xyz&child_chat_ids=subchat1,subchat2" \
  -H "Authorization: Bearer your_api_key"
```
...
curl -X GET https://api.trainly.com/debug_chat_data/chat_abc123
```
...
async function uploadFile(file, maxRetries = 3) {
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      const response = await fetch('https://api.trainly.com/extract-pdf-text', {
        method: 'POST',
        body: formData
      });
