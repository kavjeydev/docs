...
curl -X POST https://api.trainly.com/v1/chat_abc123/answer_question_stream \
  -H "Authorization: Bearer tk_your_api_key" \
  -H "Content-Type: application/json" \
...
curl -X GET https://api.trainly.com/v1/chat_abc123/info \
  -H "Authorization: Bearer tk_your_api_key"
```
...
```bash GET /v1/health
curl -X GET https://api.trainly.com/v1/health
```
...
curl -X POST https://api.trainly.com/answer_question \
  -H "Content-Type: application/json" \
  -d '{
...
curl -X POST https://api.trainly.com/answer_question_stream \
  -H "Content-Type: application/json" \
  -d '{
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      const response = await fetch(
        `https://api.trainly.com/v1/${chatId}/answer_question`,
        {
          method: 'POST',
          headers: {
